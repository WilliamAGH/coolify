...
it('requires native OCI and passive runtime acceptance before a production or staging index can be released', function () {
    ...
    foreach ($buildAndScan['steps'] ?? [] as $step) {
        if (($step['name'] ?? null) === 'Run passive control-plane runtime gate') {
            $runtimeGate = $step;
            break;
        }
    }
    ...
    expect($platformsByArchitecture)->toBe([
        ...
        ->toContain('PREFLIGHT_IMAGE="$image" tests/Integration/PassiveControlPlaneImageTest.sh')

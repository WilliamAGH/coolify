...
it('accepts the isolated browser sqlite database', function () {
    ExternalTestServicesGuard::assertSafe(
        externalTestServicesConfig(databaseName: 'database/browser-testing.sqlite'),
        false,
    );
})->throwsNoExceptions();

it('rejects another file-backed sqlite database without explicit confirmation', function () {
    ExternalTestServicesGuard::assertSafe(
        externalTestServicesConfig(databaseName: 'database/inherited.sqlite'),
        false,
    );

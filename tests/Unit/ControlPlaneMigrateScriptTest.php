...
it('merges env files key by key and never copies the source wholesale', function () {
    ...
    expect($script)
        ...
        ->toContain('target(retained-browser-port-pin)')
...
it('refuses unsupported enrollment and generation state before capture or restore mutation', function () {
    ...
    expect($script)
        ...
        ->toContain("WHERE proxy::jsonb ? '\${key}'")
        ->toContain("(proxy::jsonb ? 'control_plane_proxy_enrollment')::text")
        ->toContain("(proxy::jsonb ? 'control_plane_generation_promotion')::text")

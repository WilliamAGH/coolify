...
it('completes a consumed first-adoption drain after its worker crashes before durable completion', function (): void {
    config(['constants.ssh.mux_enabled' => false]);
    Queue::fake();
    Notification::fake();

    $filesystem = new Filesystem;
    $temporaryRoot = sys_get_temp_dir().'/coolify-consumed-first-adoption-'.bin2hex(random_bytes(8));
    $fakeBin = $temporaryRoot.'/bin';
    $baseConfigPath = $temporaryRoot.'/coolify';
    $filesystem->mkdir([$temporaryRoot, $fakeBin], 0700);
    config(['constants.coolify.base_config_path' => $baseConfigPath]);
    ...
    try {
        ...
        $scenario->state->update([
            ...
            'operation_drain_started_at' => $drainDeadline->clone()->subMinute(),
            'operation_drain_deadline_at' => $drainDeadline,
            'operation_drain_last_observed_connections' => 0,
            'operation_drain_observed_at' => now()->subMinute(),
        ]);
        $scenario->deployment->update([
            'status' => ApplicationDeploymentStatus::IN_PROGRESS->value,
        ...
        expect($expectedState?->serialize())->toBe($configuration->state->serialize());
        $expectedState ??= throw new RuntimeException('The consumed first-adoption fixture has no routed candidate state.');
        $replacementState = $expectedState->withMutationOwner(BlueGreenRecoveryScenario::OPERATION_UUID);
        $legacyTarget = new BlueGreenContainerExpectation(
            name: (string) $scenario->state->fresh()->operation_previous_container_name,
            dockerId: BlueGreenRecoveryScenario::LEGACY_ID,
...
SH);
        ...
        $completedDrain = executeSpentFirstAdoptionRemoteScript(
            $writer->fencedDestinationCommandFor(
                proxyPath: $proxyPath,
                managedFilename: $configuration->managedFilename,
                expectedState: $expectedState,
        ...
        $completedDrainDiagnostics = implode(PHP_EOL, [
            'exit='.$completedDrain->getExitCode(),
            'stdout='.$completedDrain->getOutput(),
            'stderr='.$completedDrain->getErrorOutput(),
            'docker='.implode(' | ', file($dockerLog, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) ?: []),
        ]);
        expect($completedDrain->isSuccessful())->toBeTrue($completedDrainDiagnostics)
            ->and(is_file($journalPath))->toBeFalse()
            ->and(trim((string) file_get_contents($legacyStatusPath)))->toBe('exited')
            ->and(BlueGreenProxyState::parse((string) file_get_contents($statePath))->serialize())
            ->toBe($replacementState->serialize())
            ->and($scenario->state->fresh()->phase)->toBe(BlueGreenDeploymentPhase::DRAINING)
            ->and($scenario->state->fresh()->operation_drain_last_observed_connections)->toBe(0)
            ->and($scenario->state->fresh()->operation_drain_deadline_at?->isPast())->toBeTrue()
        ...
        // Running twice models both the crashed worker's first recovery and a
        // later stale-dispatch replay. Once the remote drain has completed, both
        // must converge to the same terminal durable owner rather than parking
        // it or scheduling another bounded drain attempt.
        (new ResumeBlueGreenDrainingDeploymentJob($scenario->deployment->id))->handle();
        (new ResumeBlueGreenDrainingDeploymentJob($scenario->deployment->id))->handle();

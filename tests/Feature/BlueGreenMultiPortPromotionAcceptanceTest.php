...
it('promotes two public routes across two backend ports through production labels and durable drain completion', function () {
    ...
    expect($claim->previousActiveColor)->toBe(BlueGreenDeploymentColor::GREEN)
        ...
        ->and($finalState->inactive_retirement_stopped_at)->toBeNull()
        ->and($finalState->inactive_retirement_not_before_at->isPast())->toBeTrue()
        ->and($finalState->inactive_retirement_attempts)->toBe(0)
        ->and($finalQueue->status)->toBe(ApplicationDeploymentStatus::IN_PROGRESS->value)
        ->and($finalQueue->finished_at)->toBeNull()

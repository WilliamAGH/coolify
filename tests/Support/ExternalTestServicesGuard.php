...
final class ExternalTestServicesGuard
{
    public static function assertSafe(Repository $config, bool $externalServicesConfirmed): void
    {
        ...
        $usesIsolatedSQLiteDatabase = $databaseConnection === 'testing'
            && ($databaseConfiguration['driver'] ?? null) === 'sqlite'
            && in_array(
                $databaseConfiguration['database'] ?? null,
                [':memory:', 'database/browser-testing.sqlite'],
                true,
            );
        ...
        if ($usesIsolatedSQLiteDatabase && $usesArrayCache) {
            return;
        }

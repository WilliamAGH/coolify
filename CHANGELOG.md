...
- *(deps)* Take the patched nanoid transitive resolution
...
- *(recovery)* Compare locked owner identity without cross-type columns
...
- *(status)* Resolve active blue-green containers in batches
- *(release)* Support nested attested OCI archives

## [4.13.1-fork] - 2026-07-20

### 🚀 Features

- *(deployments)* Fence queued blue-green admission
- *(notifications)* Describe blue-green recovery outcomes
- *(proxy)* Support side-effect-free config generation
- *(proxy)* Compile control-plane listener handoff
- *(proxy)* Compile control-plane Traefik routes
- *(proxy)* Fence managed Traefik documents
- *(proxy)* Persist fenced enrollment state
- *(proxy)* Plan exact enrollment rollback
- *(proxy)* Prepare exact Traefik enrollment
- *(proxy)* Hand off APP_PORT to Traefik
- *(proxy)* Verify dual-route enrollment proof
- *(proxy)* Authenticate control-plane health proof
- *(proxy)* Attest exact route configuration
- *(proxy)* Resume control-plane enrollment
- *(proxy)* Expose fenced enrollment resume
- *(proxy)* Preserve control-plane route fragments
- *(proxy)* Resume exact enrollment rollback
- *(proxy)* Persist exact candidate backend set
- *(proxy)* Prepare enrollment from canonical host
- *(proxy)* Prove candidate members before routing
- *(proxy)* Prove candidate route convergence
- *(queue)* Freeze proxy mutation admission
- *(proxy)* Persist generation promotion state
- *(control-plane)* Drain attested predecessor generations
- *(proxy)* Attest full generation drain state
- *(proxy)* Reserve exact generation promotions
- *(proxy)* Hand off generation writer authority
- *(proxy)* Prove and freeze successor generation
- *(proxy)* Switch generation routes atomically
- Add durable deployment activation handoff
- Authenticate deployment activation payloads
- Isolate deployment activation mutations
- *(queue)* Add explicit payload diagnostics
...
- *(release)* Harden fork publication and deployment
- *(build)* Align production image with PHP 8.5
- *(ci)* Preserve required validation context
- *(ci)* Scope release workflow policy checks
- *(ci)* Provide browser Redis and tier shell lint
- *(test)* Share browser SQLite state
- *(deployments)* Preserve blue-green cleanup provenance
- *(deployments)* Drain serialized fan-out lanes
- *(deployments)* Release queues on cancellation failures
- *(notifications)* Snapshot blue-green delivery context
- *(deployments)* Verify swarm convergence
- *(routing)* Stream direct-origin curl config over stdin
- *(deployments)* Fence blue-green destination mutations
- *(blue-green)* Fence deletion and cancellation ownership
- *(blue-green)* Reconcile idle route drift
- Fence manual blue-green stops
- *(deployments)* Make blue-green handoff continuously safe
- *(blue-green)* Fence deletion and cancellation ownership
- *(blue-green)* Close lifecycle ownership gaps
- *(blue-green)* Converge interrupted lifecycle recovery
- *(blue-green)* Repair steady route drift
- *(blue-green)* Recover finalized draining deployments
- *(blue-green)* Retire stopped legacy containers
- *(blue-green)* Bound inactive color retention
- *(blue-green)* Notify lifecycle interventions
- *(blue-green)* Attest PostgreSQL retention defaults
- *(blue-green)* Reject unapproved deployment indexes
- *(release)* Preserve public APP_PORT compatibility
- *(proxy)* Preserve enrolled Traefik owners
- *(release)* Accept dual-stack APP_PORT bindings
- *(proxy)* Preserve enrolled listener ownership
- *(ci)* Execute Traefik runtime gate
- *(proxy)* Prove restored routes before rollback
- *(proxy)* Pin candidate proof to exact containers
- *(proxy)* Keep drain deadline immutable
- *(proxy)* Promote writer before releasing fence
- *(proxy)* Make generation rollback crash-safe
- *(proxy)* Read drain fence state canonically
- *(proxy)* Fence generation promotion recovery
- *(release)* Validate effective port tuples structurally
- *(release)* Keep fork version consumers exact
- *(ci)* Prove testing-host release inputs
- *(notifications)* Fail clearly for deleted teams
- *(proxy)* Consume the exact Traefik release
- *(proxy)* Keep rollback regeneration on predecessor
- Restore serialized proxy repair lane
- Close proxy mutation escape paths
- Fence deployment remote process ownership
- Attest deployment phase schema on PostgreSQL
- *(proxy)* Fence generation rollback recovery
- Preserve activation handoff recovery
- *(queue)* Snapshot diagnostic payloads atomically
- *(control-plane)* Scope durability barriers
- *(startup)* Wait for database before migrations
- Lock user and team rows through the deletion lifecycle
- *(user-deletion)* Preserve canonical team ownership and resources
- *(api)* Restrict write tokens to privileged team roles
- *(proxy)* Add bounded Traefik runtime attestation
- *(codex)* Launch Laravel Boost from the workspace
- *(release)* Block legacy ingress assets before fork publication
- *(release)* Make the ingress image census fail closed
...
- Batch provenance-safe database cleanup
- Return destination redirect after creation
- Make scheduled dispatch occurrences durable
- *(user-deletion)* Align resource previews with team ownership
- *(auth)* Attest forwarded rate-limit identity
- *(auth)* Domain-separate proxy identity proof
- *(deployments)* Surface force-start CAS loss
- *(api)* Fence deploy tags and domain identity
- *(proxy)* Make mutation retries executable
- *(settings)* Persist browser form submissions safely
- *(blue-green)* Serialize topology mutations
- *(settings)* Preserve application fields when enabling static mode
- *(settings)* Handle optional domains during save

### 💼 Other

- Update runtime dependencies for PHP 8.5
- *(stack)* Advance release CI onto dependencies
- *(stack)* Advance validation base and repair CI
- *(stack)* Advance PR3 onto validated release CI
- *(stack)* Advance PR3 onto browser database fix
- Advance application stack over release CI
- Reconcile concurrent control-plane recovery
- Advance user-deletion stack onto control-plane layer
- *(service)* Inngest one click service template
- *(api)* Add service-applications API to manage service applications
- Reconcile concurrent PR2 browser fixes
- Advance remediation stack through PR2
- Advance remediation stack through PR3
- Advance remediation stack through PR4
- Advance PostgreSQL fixes through PR3
- Advance PostgreSQL fixes through PR4

### 🚜 Refactor

- *(routing)* Centralize direct-origin recovery proof
- *(routing)* Unify blue-green direct-origin proof
- *(routing)* Unify blue-green direct-origin proof
- Isolate deployment runtime activation
...
- *(blue-green)* Format destination fence fixture
- *(api)* Tighten nested server secret visibility checks

### 🧪 Testing

- *(ci)* Seed canonical browser instance settings
- *(blue-green)* Preserve claimed server topology
- *(blue-green)* Provision recovery topology before claim
- *(routing)* Prove shared blue-green lifecycle verification
- *(blue-green)* Align lifecycle regression fixtures
- *(blue-green)* Provision recovery topology before claim
- *(blue-green)* Cover crash recovery boundaries
- *(blue-green)* Prove continuous handoff availability
- *(blue-green)* Satisfy PostgreSQL generation constraints
- *(blue-green)* Provision retention catalog fixtures
- *(proxy)* Require live Traefik route proof
- Bind legacy payloads to active database
- *(ci)* Require PostgreSQL user-deletion concurrency
...
- Seed browser instance settings canonically
- Refuse unconfirmed external services
- Allow guarded lifecycle database override
- *(browser)* Await resource persistence
- *(browser)* Synchronize auth and persistence flows
- *(browser)* Stabilize authenticated settings flows
- *(browser)* Submit the owned settings form
- *(browser)* Target stable settings form identity
- *(browser)* Await owned Livewire submit commit
- *(browser)* Wait for semantic settings success
- *(browser)* Await component-scoped settings actions
- *(browser)* Assert component-scoped settings commits
- *(blue-green)* Configure topology before operations

### ⚙️ Miscellaneous Tasks

- *(validation)* Gate the dependency stack
- *(blue-green)* Require PostgreSQL lifecycle validation
- *(blue-green)* Require PostgreSQL lifecycle validation
- *(blue-green)* Require lifecycle acceptance boundaries
- *(proxy)* Require control-plane test suite
- *(codex)* Route Laravel Boost through the shared MCP endpoint
...
- Confirm isolated lifecycle test services

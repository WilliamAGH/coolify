...
> **Role status (2026-08-12).** `popos-sf0` is no longer operated as a warm
> standby: it is the fork's dev/release-staging endpoint at
> `https://dev.coolify.haiku.host` (see
> `docs/operations/sf0-release-staging-recovery.md` for its in-place recovery
> and current isolation state). This runbook remains the authoritative
> contract for `control-plane-migrate.sh`'s `--live-standby-seed` capture
> mode and for any future production takeover, which the script itself
> references; do not follow the refresh procedure against sf0 without an
> explicit new standby designation.

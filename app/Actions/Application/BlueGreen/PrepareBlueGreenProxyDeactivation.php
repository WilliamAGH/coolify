...
class PrepareBlueGreenProxyDeactivation
{
    ...
    public function handle(
        ...
        try {
            $encodedSnapshot = $preparation->deactivation->proxy_snapshot;
            if (is_array($encodedSnapshot)) {
                return BlueGreenProxyDeactivationSnapshot::decode($encodedSnapshot);
            }
            if ($encodedSnapshot !== null) {
                throw new \UnexpectedValueException('The durable proxy deactivation snapshot is not an array.');
            }
        } catch (Throwable $exception) {
            throw new BlueGreenDeactivationException(
                'The durable proxy deactivation snapshot is malformed and requires intervention.',
        ...
        if ($preparation->activeColor() === null) {
            if ($expectedState?->managedSha256 !== null) {
                throw new BlueGreenDeactivationException('A route-less deactivation still owns managed proxy bytes.');
            }
        ...
        $expectedState ??= throw new BlueGreenDeactivationException('The active route has no exact durable destination state.');
        ...
        try {
            ...
            throw new BlueGreenDeactivationException(
                'The exact managed proxy bytes could not be compiled into a safe eviction tombstone.',
                (int) $exception->getCode(),
                $exception,
            );
        }
        ...
        return DB::transaction(function () use ($preparation, $snapshot): BlueGreenProxyDeactivationSnapshot {
            ...
            $deactivation->proxy_snapshot = $snapshot->encode();
            $encryptedSnapshot = $deactivation->getAttributes()['proxy_snapshot'] ?? null;
            if (! is_string($encryptedSnapshot)
    ...
    private function readSource(
        ...
        $output = ExecuteBlueGreenDeactivationRemoteCommand::run(
            $server,
            implode("\n", [
                $this->bootAssertion($expectedServerBootId),
                '{',
                $writer->attestStateCommandFor($proxyPath, $managedFilename, $expectedState),
                '} >/dev/null',
                $this->bootAssertion($expectedServerBootId),
        ...
        if (! is_string($destinationClockObservedAt)
            ...
            throw new BlueGreenDeactivationTransportException('The exact active managed proxy bytes could not be captured; the durable operation remains resumable.');
        }
    ...
    private function assertExpectedState(
        ...
        if ($expectedState->applicationUuid !== (string) $application->uuid
            || $expectedState->destinationId !== $preparation->destination->id
            || $expectedState->routingRevision !== $preparation->routingRevision()
            || $expectedState->activeColor !== $preparation->activeColor()
            || $expectedState->managedSha256 === null
            || ! hash_equals($expectedState->managedSha256, $sourceSha256)) {
            throw new BlueGreenDeactivationException('The active managed proxy bytes do not match the exact durable destination state.');
        }
    }
    ...
    private function attestState(
        ...
        ExecuteBlueGreenDeactivationRemoteCommand::run($server, implode("\n", [
            $this->bootAssertion($expectedServerBootId),
            $writer->attestStateCommandFor($server->proxyPath(), $managedFilename, $expectedState),
            $this->bootAssertion($expectedServerBootId),
        ]));
    }

    private function bootAssertion(string $expectedServerBootId): string
    {
        return (new ReadBlueGreenServerBootIdentity)->assertionCommandFor($expectedServerBootId).' || exit 75';
    }

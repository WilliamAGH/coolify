...
final class AbortPreparedControlPlaneProxyEnrollment
{
    ...
    private function assertAbsentOrRecoverableDirectory(
        ...
        $output = $execute(
            ...
            .'trusted_owner() { owner_uid=$(if stat -c "%u" "$1" >/dev/null 2>&1; then stat -c "%u" "$1"; else stat -f "%u" "$1"; fi) || return 1; '
            .'[ "$owner_uid" = 0 ] || [ "$owner_uid" = 9999 ] || [ "$owner_uid" = "$current_uid" ]; }; '
            .'private_permissions() { permissions=$(if stat -c "%a" "$1" >/dev/null 2>&1; then stat -c "%a" "$1"; else stat -f "%Lp" "$1"; fi) || return 1; '
            .'case "$permissions" in ???|????) ;; *) return 1 ;; esac; case "$permissions" in *[!0-7]*) return 1 ;; esac; '
            ...
            .'case "${group_permissions}${other_permissions}" in *[2367]*) return 1 ;; esac; }; '
            .'if [ ! -e "$managed_state_directory" ] && [ ! -L "$managed_state_directory" ]; then '
            .'printf '.escapeshellarg(self::ABSENT_ARTIFACT."\n").'; '
            .'elif [ -d "$managed_state_directory" ] && [ ! -L "$managed_state_directory" ]; then '
            .'trusted_owner "$managed_state_directory" && private_permissions "$managed_state_directory" || exit 1; '
            .'[ -f "$lock_path" ] && [ ! -L "$lock_path" ] && trusted_owner "$lock_path" && private_permissions "$lock_path" || exit 1; '
            ...
            .'[ "$state_links" = 1 ] && trusted_owner "$state_path" || exit 1 ;; '
            .'"$managed_state_directory"/.managed-traefik-document.*) '
            .'[ -d "$state_path" ] && [ ! -L "$state_path" ] && trusted_owner "$state_path" || exit 1; '
            ...
            .'for scratch_child in "$state_path"/* "$state_path"/.*; do [ -e "$scratch_child" ] || continue; '
            .'case "${scratch_child##*/}" in .|..) continue ;; esac; trusted_owner "$scratch_child" || exit 1; done ;; '
            .'*) exit 1 ;; esac; done; '

...
final class ManagedTraefikDocumentWriter
{
    ...
    public function rollbackAbandonedInitialEnrollmentCommandFor(
        ManagedTraefikDocumentMutation $mutation,
        ManagedTraefikDocumentWriterAuthority $replacementAuthority,
        ManagedTraefikDocumentWriterAuthority $rolledBackAuthority,
    ): string {
        if (! $replacementAuthority->matchesReplacement($mutation)
            ...
            || ! $rolledBackAuthority->hasSameWriterIdentityAs($replacementAuthority)) {
            throw new InvalidArgumentException('The abandoned initial enrollment rollback authority must fence the exact replacement and predecessor document.');
        }
        ...
        return $this->commandFor(
            ...
            requiredAuthority: $replacementAuthority,
            nextAuthority: $rolledBackAuthority,
            exactAbandonedInitialEnrollmentRollback: true,
        );
    }
    ...
    private function commandFor(
        ...
        return implode("\n", [
            ...
            '    fail',
            '  fi',
            '  if [ "${COOLIFY_MANAGED_TRAEFIK_DOCUMENT_CRASH_AFTER_SIDECAR:-}" = 1 ]; then exit 75; fi',
            ...
            '}',
            'assert_reconcilable_legacy_scratch_directory() {',
            '  legacy_scratch_path=$1',
            '  legacy_scratch_prefix="$state_directory/.managed-traefik-document."',
            '  legacy_scratch_suffix=${legacy_scratch_path#"$legacy_scratch_prefix"}',
            '  test "$legacy_scratch_suffix" != "$legacy_scratch_path" || fail',
            '  test "${#legacy_scratch_suffix}" = 6 || fail',
            '  case "$legacy_scratch_suffix" in *[!A-Za-z0-9]*) fail ;; esac',
            '  durable_remote_assert_owned_directory "$legacy_scratch_path" || fail',
            '  for legacy_scratch_entry in "$legacy_scratch_path"/* "$legacy_scratch_path"/.*; do',
            '    if [ ! -e "$legacy_scratch_entry" ] && [ ! -L "$legacy_scratch_entry" ]; then continue; fi',
            '    case "$legacy_scratch_entry" in',
            '      "$legacy_scratch_path/."|"$legacy_scratch_path/..") ;;',
            '      "$legacy_scratch_path/expected-sidecar"|"$legacy_scratch_path/replacement-sidecar"|"$legacy_scratch_path/replacement-payload"|"$legacy_scratch_path/expected-journal"|"$legacy_scratch_path/forward-journal"|"$legacy_scratch_path/authority-required"|"$legacy_scratch_path/authority-next"|"$legacy_scratch_path/predecessor-document") durable_remote_assert_owned_regular "$legacy_scratch_entry" || fail ;;',
            '      *) fail ;;',
            '    esac',
            '  done',
            '}',
            'cleanup_reconcilable_legacy_scratch_directories() {',
            '  legacy_scratch_removed=false',
            '  for legacy_scratch_candidate in "$state_directory"/.managed-traefik-document.*; do',
            '    if [ ! -e "$legacy_scratch_candidate" ] && [ ! -L "$legacy_scratch_candidate" ]; then continue; fi',
            '    if [ ! -d "$legacy_scratch_candidate" ] || [ -L "$legacy_scratch_candidate" ]; then continue; fi',
            '    assert_reconcilable_legacy_scratch_directory "$legacy_scratch_candidate"',
            '    for legacy_scratch_child_name in expected-sidecar replacement-sidecar replacement-payload expected-journal forward-journal authority-required authority-next predecessor-document; do',
            '      legacy_scratch_child="$legacy_scratch_candidate/$legacy_scratch_child_name"',
            '      if [ -e "$legacy_scratch_child" ] || [ -L "$legacy_scratch_child" ]; then durable_remote_remove "$legacy_scratch_child" "$legacy_scratch_candidate" || fail; fi',
            '    done',
            '    rmdir "$legacy_scratch_candidate" || fail',
            '    legacy_scratch_removed=true',
            '  done',
            '  if [ "$legacy_scratch_removed" = true ]; then sync "$state_directory" || fail; fi',
            '}',
            'assert_exact_abandoned_initial_enrollment_census() {',
            '  for state_path in "$state_directory"/* "$state_directory"/.*; do',
            '    if [ ! -e "$state_path" ] && [ ! -L "$state_path" ]; then continue; fi',
            '    case "$state_path" in',
            '      "$state_directory/."|"$state_directory/.."|"$sidecar_path"|"$lock_path"|"$journal_path"|"$artifact_path"|"$authority_path") ;;',
            '      "$state_directory/.managed-traefik-document.$filename."*) durable_remote_assert_owned_regular "$state_path" || fail ;;',
            '      "$state_directory/.managed-traefik-journal.$filename."*) durable_remote_assert_owned_regular "$state_path" || fail ;;',
            '      "$state_directory/.managed-traefik-document."*) assert_reconcilable_legacy_scratch_directory "$state_path" ;;',
            '      *) fail ;;',
            '    esac',
            '  done',
            '}',
            'assert_exact_abandoned_initial_enrollment_state() {',
            '  validate_artifact',
            '  if [ -e "$authority_path" ] || [ -L "$authority_path" ]; then',
            '    authority_matches "$authority_path" "$authority_next_file" || fail',
            '    if [ -e "$journal_path" ] || [ -L "$journal_path" ]; then',
            ...
            '    if [ -e "$journal_path" ] || [ -L "$journal_path" ]; then fail; fi',
            '    document_matches "$document_path" "$original_replacement_document_sha" || fail',
            '    sidecar_matches "$sidecar_path" "$original_replacement_sidecar_base64" "$expected_sidecar_file" || fail',
            '  fi',
            '}',
            '',
            'if [ "$exact_abandoned_initial_enrollment_rollback" = true ]; then',

...
enum ControlPlaneGenerationPromotionPhase: string
{
    ...
    public function assertCanTransitionTo(self $next): void
    {
        ...
        $allowed = match ($this) {
            ...
            self::Retiring => [self::FenceReleasing, self::RollingBack, self::InterventionRequired],
            self::FenceReleasing => [self::WriterPromoting, self::RollingBack, self::InterventionRequired],
            self::WriterPromoting => [self::Unfreezing, self::RollingBack, self::InterventionRequired],
